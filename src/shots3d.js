...
export function createShots({ colour = "#a8913e" } = {}) {
  ...
  /** Send a ball from `from` ([x, y, z]) at `velocity` ([vx, vy, vz]), both in the scene's frame. */
  ...
  function step(now) {
    for (let i = balls.length - 1; i >= 0; i--) {
      if ((now - balls[i].born) / 1000 > SHOWN_S + FADE_S) balls.splice(i, 1);
    }
    mesh.count = balls.length;
    balls.forEach((ball, i) => {
      const t = (now - ball.born) / 1000;
      place.set(
        ball.from[0] + ball.velocity[0] * t,
        ball.from[1] + ball.velocity[1] * t - 0.5 * GRAVITY * t * t,
        ball.from[2] + ball.velocity[2] * t
      );
      turn.setFromAxisAngle(ball.axis, ball.spin * t);
      size.setScalar(smooth(t / EMERGE_S) * (1 - smooth((t - SHOWN_S) / FADE_S)));
